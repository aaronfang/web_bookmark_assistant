...
项目已完成 Phase 0 和 Phase 1，正在进行 Phase 2。扩展会只读导入 Chrome 原生书签到本地 IndexedDB，在侧边栏提供标题、URL 和文件夹搜索，在管理页按原始层级浏览文件夹并检测重复 URL，同时监听 Chrome 书签变化实时刷新结果。AI 接口默认禁用，不会上传书签内容。
