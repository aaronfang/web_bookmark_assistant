...
项目已完成 Phase 0 和 Phase 1，Phase 2 的重复检测、只读健康检查、链接探测和安全移动基础设施已完成，正在进入 Phase 3。扩展会只读导入 Chrome 原生书签到本地 IndexedDB，在侧边栏提供标题、URL 和文件夹搜索，在管理页按原始层级浏览文件夹、检测重复 URL 并执行只读健康检查，同时监听 Chrome 书签变化实时刷新结果。AI 接口默认禁用，不会上传书签内容。
...
快速收藏支持用户主动读取当前页面的 description、选中文本和正文摘录。正文优先来自 article/main 主内容区域，会在本地移除脚本、导航、页眉页脚、表单等常见噪声并限制为 6,000 字；摘录不写入书签或本地数据库，只有用户点击 AI 摘要或分类按钮时才会交给已配置的 Provider。

AI 分类结果会展示内容类型、候选标签、文件夹建议、置信度和判断依据，不会直接覆盖标签。用户可在快速收藏中明确采用标签；未采用的结果会随书签保存到本地“AI 待整理箱”，其中低于 70% 的建议会标为低置信度，可稍后应用或忽略。

AI 文件夹建议会优先从最多 100 个现有 Chrome 文件夹路径中选择，扩展还会在本地要求唯一的精确或路径后缀匹配。如果没有合适目录，AI 可以建议在现有父目录下新增一个子目录；用户必须确认名称和父级，目录只在最终保存书签时查重并创建，保存失败会删除本次产生的空目录。重名、模糊或低置信度建议不会自动创建目录，也不会移动已有书签。
...
- Manifest 当前申请 `bookmarks`、`sidePanel`、`alarms`、`activeTab`、`scripting` 和 `<all_urls>`；其中 `<all_urls>` 仅用于用户主动或低频定时的只读链接探测，`activeTab`/`scripting` 仅用于用户点击后读取当前页面元数据。
