...
当前进度（2026-07-17）：已加入快速收藏入口，可保存当前页面标题、URL、目标文件夹、标签、备注和阅读状态；用户主动点击后可读取页面 description 元标签和选中文本，作为后续摘要与分类输入。已定义 AI Provider 的内容输入、摘要、分类建议和禁用实现，并加入可选的本地 Ollama Provider（请求超时、错误和 JSON 响应校验）；管理页已提供本地 AI 设置面板，可选择禁用或配置 Ollama 并测试连接；默认不会发起任何 AI 请求。
