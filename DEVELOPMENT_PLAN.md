...
当前进度（2026-07-16）：已完成 URL 规范化和只读重复候选检测，区分原始 URL 完全相同与去除常见追踪参数、片段后的规范化相同；已加入空文件夹、保守的低质量标题和超过 730 天长期收藏候选检查。文件夹移动建议采用同主机名至少 4 条、目标文件夹至少 3 条且占比至少 75% 的保守规则，提供当前位置、建议位置、置信度和依据；支持多选后预览批次涉及的书签、来源及目标文件夹。操作日志采用批次与逐书签步骤两层模型，移动计划强制关联快照并记录原父文件夹、原索引和目标位置；受控执行器已实现位置校验、逐步状态记录和失败自动回滚，并已接入整理建议 UI：用户点击“创建快照并执行移动”后才会下载 JSON 快照、创建计划并执行，失败时自动逆序回滚。真实 UI 执行与故障回滚均通过隔离 Chrome Profile E2E。测试 harness 仅在条件构建的 `.output-e2e` 中存在，生产构建会硬性检查其不存在。管理 Dashboard 已按概览、文件夹、诊断、整理建议和备份拆分为导航式单面板布局。所有候选会随 Chrome 书签事件实时刷新，当前不提供自动删除或合并操作。
