...
当前进度（2026-07-16）：已完成 URL 规范化和只读重复候选检测，区分原始 URL 完全相同与去除常见追踪参数、片段后的规范化相同；已加入空文件夹、保守的低质量标题和超过 730 天长期收藏候选检查。文件夹移动建议采用同主机名至少 4 条、目标文件夹至少 3 条且占比至少 75% 的保守规则，目前仅提供当前位置、建议位置、置信度和依据预览。管理 Dashboard 已按概览、文件夹、诊断、整理建议和备份拆分为导航式单面板布局。所有候选会随 Chrome 书签事件实时刷新，当前不提供自动删除、移动或合并操作。
